...
export class SqliteAccountStore implements AccountStore {
  ...
  async create(account: Account): Promise<void> {
    this.db
      .prepare(
        'INSERT INTO accounts (player_id, username, password_hash, name, created_at) VALUES (?, ?, ?, ?, ?)',
      )
      .run(account.playerId, account.username, account.passwordHash, account.name, account.createdAt);
  }

  async findByUsername(username: string): Promise<Account | undefined> {
    return this.row(this.db.prepare('SELECT * FROM accounts WHERE username = ?').get(username));
  }
